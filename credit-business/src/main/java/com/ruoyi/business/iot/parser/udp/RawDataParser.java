...
public class RawDataParser {
    ...
    public static byte[] parse(byte[] receivedData, String aesKey) throws Exception {
        ...
        byte snLength = receivedData[5]; // 倒数第二个字节是 CS1
...
//        short totalLength = buffer.getShort(2); // 从第2字节读取总长度（小端序）
        ...
        int position = 2 + 2 + 1 + 1 + snLength; // 跳过：起始标志(2B) + 总长度(2B) + 协议版本(1B) + sn长度 + sn
        buffer.position(position); //

        // 加密体长度 = 总长度字段值 - 2（因为总长度包含 协议 ,sn长度,sn, CS1 和 DD 字节,剩下的才是加密体）
        byte[] encryptedBody = new byte[totalLength - 1 - 1 - snLength - 1 - 1];
